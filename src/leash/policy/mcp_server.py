...
def authoring_guide() -> dict[str, Any]:
    ...
    return {
        ...
        "instructions": [
            ...
            "When the instruction forbids add-ons, do not write a rule facts.is_addon = false. Extraction is three-valued and reports is_addon unknown on a line with no add-on marker, so that rule turns every clean purchase into a step_up. The engine's built-in add-ons check already fails a detected add-on line. Use facts.is_addon only for an explicit add-on-specific constraint the merchant text can prove.",
