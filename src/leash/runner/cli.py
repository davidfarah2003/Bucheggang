"""Command line for one live scenario run.
    ...
    uv run python -m leash.runner.cli --scenario SCEN0101 --mandate-id TM... --evaluate decline_everything_smoke
...
def decline_everything_smoke(event: Event, state: MandateState) -> Decision:
...
def main() -> None:
    ...
    state = loop.run_loop(run["run_id"], EVALUATORS[args.evaluate], state)
