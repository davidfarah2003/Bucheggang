...
from .types import AssessmentBundle


def purchase_digest(event: Event) -> str:
    """Hash the authorization and mandate identity without sending either to a provider."""
    payload = {
...
def history_bundle(event: Event, policy: PolicyDraft, history: HistoryIndex) -> AssessmentBundle:
    ...
    return AssessmentBundle(
        ...
        purchase_digest=purchase_digest(event), policy_hash=policy.hash,
...
def validate_bundle(event: Event, policy: PolicyDraft, bundle: AssessmentBundle) -> None:
    ...
    if bundle.purchase_digest != purchase_digest(event):
