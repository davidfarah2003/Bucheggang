...
async def main() -> None:
    ...
    try:
        ...
        async with async_playwright() as pw:
            ...
            await page.evaluate("() => window.scene('lead', 'Alex writes the rules first.', 'One sentence, typed before the agent buys anything.')")
            ...
            await s.caption("The agent drafted the plan. Alex has to approve it.", 0.4)
            ...
            await s.caption("The rules are in plain English, and Alex confirms them before the first purchase.", 0.6, side=True)
            ...
            await s.caption("Rules confirmed. Now the agent can shop, within those limits.", 0.5)
            ...
            await s.say("agent ask", "The Wallet has a question for you before this goes through.", 0.1)
            ...
            await s.caption("It asks because this is a new device and a risk model flagged the activity. Alex answers.", 1.6, side=True)
            ...
            await s.say("agent bad", "Declined. Your rule says shops you already use, and this card has never bought there.", 0.7)

            # Why: the decision pipeline and the live probabilities, with the explainer on the left.
            await s.caption("Every decision is on record with its reason.", 0.3)
