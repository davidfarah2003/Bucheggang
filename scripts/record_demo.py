...
class Stage:
    ...
    async def zoom(self, x: float, y: float, s: float, ms: int = 900, hold: float | None = None):
        await self.page.evaluate("([x,y,s,ms]) => window.zoom(x,y,s,ms)", [x, y, s, ms])
        self.cue("whoosh")
...
async def main() -> None:
    ...
    try:
        async with async_playwright() as pw:
            ...
            # Sign up, zoomed on the phone so the taps read.
            await s.zoom(PHONE_CX, 330, 1.6, 700, hold=0.2)
            await s.click('[data-action="auth-mode"]', after=0.1)
            await s.type_into("#username", "alex", per_char=0.02)
            await s.type_into("#password", "demo-password-1", per_char=0.006)
            await s.click('[data-action="register"]', after=0.1)
            await s.frame.wait_for_selector("#shop-prompt")

            await page.evaluate("() => window.scene('lead', 'One sentence.', 'Limits, shops, what counts as groceries.')")
            await s.zoom(PHONE_CX, 480, 1.35, 600, hold=0.2)
            ...
            await s.tool("connect", 'agent_label="Grocery helper"', 0.2)

            await wait_for_agent(log, "connection request is waiting")
            await s.caption("The agent asks to connect. Alex taps once.", 0.7)
            await s.click('nav [data-route="wallet"]', after=0.2)
            await s.frame.wait_for_selector('[data-action="approve-pending-pair"]')
            await s.zoom(PHONE_CX, 420, 1.5, 500, hold=0.2)
            await s.click('[data-action="approve-pending-pair"]', after=0.2)
            await s.zoom_out(450)
            await s.done(0.2, sound="connect")
            await s.say("agent", "Connected. Turning your sentence into a spending plan.", 0.2)
            await s.tool("propose_task_policy", "instruction, proposal{6 rules, 1 question}")
            await wait_for_agent(log, "policy proposed")
            await s.done(0.1)
            await s.tool("wait_for_policy", "draft_id", 0.2)
            await s.caption("The plan waits in the Wallet. Only Alex can authorize it.", 0.8)
            await s.frame.wait_for_selector('[data-action="open-draft"]', timeout=15000)
            await s.click('[data-action="open-draft"]', after=0.3)
            await s.zoom(PHONE_CX, 470, 1.45, 600, hold=0.2)
            await s.caption("Plain-English rules, in Alex's words.", 0.7, side=True)
            await s.scroll(720, steps=8)
            await s.click('[data-action="answer-question"]', after=0.25)
            await s.click('[data-action="confirm"]', after=0.3)
            await s.zoom_out(450)
            await wait_for_agent(log, "policy confirmed")
            await s.done(0.3, sound="confirm")
            await s.caption("Permission active. The agent shops.", 0.6)
            await s.say("agent", "Confirmed. Ordering the weekly basket at Alpine Basket, CHF 35.00.", 0.2)
            ...
            await s.tool("wait_for_purchase", "authorization_id", 0.2)
            await s.click('[data-action="wallet-tab"][data-tab="needs"]', after=0.2)
            ...
            await s.caption("Why it asks: a new device and a risk-model flag. Alex decides, never the agent.", 2.4, side=True)
            await s.click('[data-action="resolve"][data-decision="approve"]', after=0.2)
            await s.zoom_out(450)
            await wait_for_agent(log, "customer answered")
            await s.done(0.5, sound="approve")
            await s.say("agent", "Approved. Trying a second shop for the missing items.", 0.2)
            await s.tool("buy", "mandate_id, merchant=Fresh Corner Market, total_chf=35.0")
            await wait_for_agent(log, "agent: done")
            await s.done(0.1)
            s.cue("deny")
            await s.say("agent", "Declined: shops you already use only, and this card has never bought there.", 1.6)

            # Why: the decision pipeline and the live probabilities.
            await s.caption("Every decision on record, with the reason.", 0.6)
            await s.click('nav [data-route="activity"]', after=0.3)
            await s.frame.wait_for_selector('[data-action="open-detail"]')
            await s.click('[data-action="open-detail"] >> nth=1', after=0.3)
            await s.click('[data-action="inspector"][data-tab="summary"]', after=0.25)
            await s.click('[data-action="inspector"][data-tab="pipeline"]', after=0.2)
            await s.zoom(PHONE_CX, 470, 1.6, 600, hold=0.2)
            await s.caption("Four stages: rules, cart facts, card history, risk models.", 1.2, side=True)
            await s.scroll(900, steps=14)
            await s.caption("Live models: a CatBoost history score and Jev's probabilities, shown to Alex.", 0.6, side=True)
