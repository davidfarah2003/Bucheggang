...
function renderLogin() {
  ...
  const context = hasPairLink() ? "Sign in to review an agent connection request." : "Sign in to review spending permissions sent by your shopping agent.";
...
async function renderAgents(serial) {
  ...
  if (serial !== state.serial) return;
  state.agents = agents;
  const cards = agents.map((agent) => `<article class="identity-card agent-card"><span class="section-kicker">${agent.revoked_at ? "REVOKED" : "CONNECTED"}</span><h2>${esc(agent.agent_label)}</h2><div class="agent-meta"><span>Access</span><strong>${esc(agent.scopes.join(" · "))}</strong><span>Connected</span><strong>${esc(new Date(agent.created_at).toLocaleString("en-CH"))}</strong><span>Last used</span><strong>${agent.last_used_at === null ? "Never" : esc(new Date(agent.last_used_at).toLocaleString("en-CH"))}</strong></div>${agent.revoked_at ? `<p>Access revoked ${esc(new Date(agent.revoked_at).toLocaleString("en-CH"))}.</p>` : `<div class="agent-actions"><button type="button" class="outline-button" data-action="open-agent-revoke" data-id="${esc(agent.agent_id)}">Revoke access</button></div>`}</article>`).join("");
