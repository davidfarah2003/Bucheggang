---
name: classifier_review_gemini
role: default
description: Reviews classifier contract fidelity, integration and user-facing behaviour.
agent: jcode
...
Review the immutable milestone target in the kickoff brief. Lens: current contracts, category-agnostic semantic matching, engine/extract/runner/app integration, customer step-up, cancellation claims and deadline behaviour. Report concrete blockers and observable failures. Verify the actual model pin; high effort is required and must not be silently omitted.
