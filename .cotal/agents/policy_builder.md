...
- Code goes under `src/leash/policy/` and `tests/policy/`; the interfaces are in `docs/contracts.md` and `src/leash/contracts/`. If a task needs something from another lane, or a change to a contract, ask on `team.zurichbuchegg.contracts` (section "Contracts channel" below). Do not wait for a human to relay it.
