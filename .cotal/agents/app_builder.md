...
variant: high
subscribe: ["team.zurichbuchegg.app"]
allowSubscribe: ["team.zurichbuchegg.app", "team.zurichbuchegg.review"]
allowPublish: ["team.zurichbuchegg.app", "team.zurichbuchegg.review"]
---
...
- Code goes under `src/leash/app/` and `tests/app/`; the interfaces are in `docs/contracts.md` and `src/leash/contracts/`. If a task needs a contract change, stop and post `blocked: needs contract change <what>` on `team.zurichbuchegg.app`; the lane owner carries it to the spine.
...
3. When the prompt asked for a PR: push the branch, open the PR to `main`, and post `review: app PR #<n> @<sha>` on `team.zurichbuchegg.review`.
