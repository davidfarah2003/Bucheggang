...
- Code goes under `src/leash/app/`; the interfaces are in `docs/contracts.md` and `src/leash/contracts/`. If a task needs something from another lane, or a change to a contract, ask on `team.zurichbuchegg.contracts` (section "Contracts channel" below). Do not wait for a human to relay it.
...
1. Append one line to the plan's Log: `<date time> app_builder: <what>, <how you tried it>, @<sha>`. Commit it with the code.
