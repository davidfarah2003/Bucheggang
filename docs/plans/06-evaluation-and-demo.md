# 06 Evaluation and demo

- Status: report generator merged in #38. Reconciled labels are docs/eval/labels.csv (strict reading, #55) and the labelled report is docs/eval/report-labelled.md: 43/45 agreement, autonomous error 1/43. The 45-input real Jev exercise passed on the merged classifier head (plan 02 Log, 13:25). The demo video is take 26 on main and on release demo-2026-09-25.
- Owner: everyone; Rishabh drives the pitch
- Channel: `team.zurichbuchegg` (spine)
- Papers: [Selective Conformal Risk Control](../papers/2512.12844v2.pdf) (report the two numbers: how often the system decides alone, and how often it is wrong when it does; its guarantees need more labelled, exchangeable data than 45 attempts, so no guarantee is claimed), [CaMeL](../papers/2503.18813v2.pdf) (report lost utility next to blocked attacks)
- Event: jury criteria and slots in [primary-findings.json](../research/2026-09-24/primary-findings.json) (JURY, SUBMIT); comparable projects in [wallet-control-research.html](../research/2026-09-24/wallet-control-research.html)

## Goal

Labels to check decisions against, a report the pitch can quote, and a demo that runs the same way twice.

## Labels

`docs/eval/labels.csv`: one row per public attempt, `authorization_id, expected, reason, notes`. Two people label independently tonight from `purchase_attempts.csv`, `purchase_attempt_items.csv`, `merchants.csv` and the scenario instruction, then reconcile disagreements in a third column. The organizers publish no answer key; these are our labels and the report says so. Known judgement calls to settle while labelling: AU0016 (return terms missing), the SCEN0001 split order, "a shop I use regularly" for an unseen merchant.

## Report

`docs/eval/report.md`, produced by `scripts/report.py` from `scripts/replay.py` output:

- decisions per scenario: approve, decline, step_up counts;
- agreement with labels, with every disagreement listed and explained;
- autonomy rate (share decided without a step-up) and error rate on those, per scenario;
- extract lane's corpus table (pass 1; there is no model pass, docs/idea Decision pipeline);
- latency p50 and p99 for offline extraction/evaluation and the available live measurements; complete live end-to-end timing remains unavailable until both ends are recorded;
- attacks in SCEN0004 stopped or escalated, and ordinary purchases in SCEN0000 to SCEN0002 approved without a question.

Numbers come from files in `docs/eval/`; the report links to them. No number appears in the pitch that is not in the report. [run-report.md](../run-report.md) documents explicit inputs, labels-pending operation and measurement limits. [live-demo-checklist.md](../live-demo-checklist.md) separates observed evidence from the customer flow still to be exercised.

## Demo script (four minutes of content, cut to the slot)

This is the intended sequence. The public attempt IDs below identify offline examples, not selectable live purchases. Use the live bootstrap's team scenario IDs, the customer's actual choices and the resulting decisions. Do not announce an approval or fixed latency before observing it.

1. The external shopping agent proposes a `SCEN0002` draft through MCP and opens the Wallet at `/app/?draft_id=<id>`. Review the saved draft and examples, answer the return-terms question, and confirm. Show the `mandate_id` the agent receives. The Wallet also works directly without any built-in Harness.
2. Show an ordinary purchase's actual outcome and judge panel. Quote the measured timing. The early `SCEN0002` approval is an offline reference; missing live-card history may cause a step-up.
3. A manipulated purchase from `SCEN0004`: injected merchant text is shown verbatim on the panel, flagged, and the decision is driven by the rule it tried to override.
4. A `step_up` on the phone: AU0016, return terms not stated; the customer rejects; the next attempt at the same merchant is declined as a re-quote.
5. Revoke on the policy page; the following purchase is declined with `mandate_revoked`.
6. Close on the report's two numbers per scenario.

Assets: `docs/screens/` screenshots of the mobile-first Wallet demo, the agent-side terminal transcript from plan 01 step 7, a 6-slide deck in `docs/pitch/`. Harness screenshots are later product work.

## Steps

1. Tonight: labels by two people, reconciled.
2. Friday 09:00: `scripts/report.py` and the first report from the offline replay.
3. Friday 10:30: full live dry run. The demo is live; there is no recorded fallback (AGENTS.md section 6).
4. Friday 11:00: deck; book the expert-round slot before 12:00.
5. After the expert round: fold Q&A findings; final submission 17:30.

## How we check it works

- The demo runs twice in a row against the live API with the same outcomes.
- The report's numbers match the replay CSV it links.
- Nobody in the room can find a scenario ID in `src/`.

## Decisions

- Keep the expert pitch within its 1-minute slot and show a concise live flow. Screenshots from a real run may support the pitch deck, but do not replace the live demo.

## Log

(one line per finished task: date time, who, what, how it was tried, sha)

2026-09-24 20:32 oskar1: clarified that the current demo uses an external shopping agent through MCP and opens the standalone Wallet with the saved draft ID; a built-in Harness is later work.

2026-09-25 00:09 david_orch: report generator, evidence-linked report and live checklist implemented @3852542. `uv run python scripts/replay.py --all` with the five explicit drafts documented in run-replay.md wrote the new 45-row replay: 11 approve, 32 decline, 2 step_up; local total p50 0.248 ms, p99/max 0.462 ms. `uv run python scripts/report.py` read that replay plus four live CSVs (50 finalized, 49 local outcomes, one platform-only timeout), measured 145/145 annotated extraction fields and 7/7 authored adversarial instruction recall, and wrote docs/eval/report.md with labels pending. Explicit provisional labels-b @4e15565 exercised all labelled calculations and disagreement rows in an ignored scratch report; no provisional label metrics were put in the published report. Missing explicit labels and mismatched label/replay mode exited 1; omitted --labels-mode and existing output exited 2; the existing report hash was unchanged. labels-a @c3afec9 initially failed on its trailing comment row, reported to its owner; no input repair or fallback was applied by the reporter. No simulator request, customer answer, test suite or linter. Human reconciliation, full Wallet E2E, complete live end-to-end timing and repeated live demo remain open. Review pending.

2026-09-25 david_orch: real Jev exercise on core71f119c and standalone producer0940fd7 with evaluation-only CatBoost e4815c5. Initial driver exited1 after6 composed inputs when request7 failed activity probability normalization; one separate diagnostic request passed the unchanged parser. Fresh recorded driver exited1 after35 composed inputs when request36 returned spend probabilities unusual0.14, ordinary0.80 and unclear0.05, sum0.99. Total44 actual provider requests; neither45-input batch completed. No retries within batches, normalized probabilities, substitute decisions, simulator requests or customer answers. Full model-off scripts/replay.py --all still returned45 rows11/32/2, local p50 total0.239ms p99/max0.421ms. Evidence: docs/eval/jev-public-2026-09-25.md and linked partial CSVs/JSON. Strict validation and the model-enabled release gate remain.
2026-09-25 oskar7: regenerated `docs/eval/replay-2026-09-25-all.csv` from the five explicit draft paths in `docs/run-replay.md` on main ec5bb96; `uv run python scripts/replay.py --all --policy SCEN0000=docs/eval/replay-policies/SCEN0000.json --policy SCEN0001=docs/eval/replay-policies/SCEN0001.json --policy SCEN0002=docs/samples/scen0002_draft.json --policy SCEN0003=docs/eval/replay-policies/SCEN0003.json --policy SCEN0004=docs/eval/replay-policies/SCEN0004.json --output docs/eval/replay-2026-09-25-all.csv` exited 0: 45 rows, 11 approve, 32 decline, 2 step_up. Decisions match the prior CSV; AU0040 reasons are now `item_mismatch|injected_instructions` instead of `item_mismatch|unrequested_item|injected_instructions`, and SCEN0004 draft hash is now `3a83840408674d175a5ace5a629983fd0f08e9162bdba814d6d17e36fc603870` instead of `743b2c4ada5e80acdd0eb1248bcfc6f96a0bb011cc3173cb187058158552a7ce` after the `facts.is_addon = false` rule was removed in policy @72e7b18. No simulator call, customer answer or label choice.

2026-09-25 david_orch: independent Chrome exercise of frozen app source1c5d61c used the actual Uvicorn API and disposable accounts/store. Registration201, logout then session401, wrong-password error, correct-password login, persistent session after reload, empty Connected agents and Wallet/Activity views, unknown pairing404 and Shop escape all observed. Measured viewport/document widths390/390 and320/320; browser storage empty. Probe corrections and scope in docs/eval/identity-browser-2026-09-25.md. Owned tab/server closed, task exited0, temporary store removal verified. No pairing approval, mandate confirmation, purchase answer, provider or simulator request.

2026-09-25 03:25 david_main: labels reconciliation prepared on lane/eval-labels. Labeler A (grok-4.7, lane/eval-labels-a @4df266c) and labeler B (gpt-5.6-sol, lane/eval-labels-b @4e15565) agree on 37 of 45 attempts. Six rows (AU0019, AU0023, AU0038, AU0040, AU0042, AU0045) are set to decline under the sequential reading, one successful purchase per request. Six rows remain a human call and are written both ways: AU0016 (return window not stated) and the SCEN0003 group AU0027, AU0028, AU0029, AU0030, AU0033 (new device at a shop never used). `docs/eval/labels-candidate-strict.csv` declines all six (engine agrees on all six, 43/45 label agreement, autonomous error 1/43). `docs/eval/labels-candidate-ask.csv` step_ups all six (engine disagrees on all six). Either way the two remaining disagreements are AU0035 (labels approve, engine item_mismatch from the replay policy wording "27-inch monitor" versus catalogue "27-inch computer monitor") and AU0040 (labels decline under the sequential reading, engine step_up on injected_instructions). David picks one candidate, it becomes docs/eval/labels.csv, and `uv run python scripts/report.py --replay docs/eval/replay-2026-09-25-all.csv --labels docs/eval/labels.csv --labels-mode sequential --live <four docs/eval/live-*.csv> --output docs/eval/report-labelled.md` builds the report; both builds tried at faff60e, exit 0. Replay CSV regenerated on faff60e @061e6e1, 45 rows 11/32/2, AU0040 reason codes now item_mismatch|injected_instructions.
2026-09-25 04:45 david_main: replay and report regenerated at 033a016 so the recorded extractor hash (4d0cc8c2, facts.py after #59 and #64) and engine version (leash-engine 0.5) match the code; decisions and reasons unchanged, 45 rows 11 approve, 32 decline, 2 step_up. Same replay.py and report.py commands as the 03:25 line.
2026-09-25 07:19 david_main: lane/eval-labels rebased onto main ffdb74b (plan 06 Log conflict, both sides kept), CRLF stripped from the two candidate CSVs, @a392e6b; still waiting on the strict or ask word
2026-09-25 11:05 david_main: demo video recorded. app/demo-stage.html embeds the real Wallet in a phone frame beside a mirrored agent chat; scripts/record_demo.py drives the Wallet with Playwright while scripts/flow_sim.py --manual runs the agent over MCP with the models on. Take 4 on a wiped store: register, connect approved, propose, confirm, buy at Alpine Basket step_up (new_device, model_history_uncertain) answered in the Wallet as approve, buy at Fresh Corner Market decline (unfamiliar_merchant), activity and the pipeline tab with the CatBoost and Jev bars. Output docs/demo/leash-user-flow.mp4 58 s at 1600x900 30 fps, gif 800 px. The step-up answer in the recording is a scripted browser tap, not a person. Commits ead3e8e, 33e8b45.
2026-09-25 11:50 david_main: demo video reworked after David's review. Camera zooms (stage clips scrolling so Playwright's scroll-into-view cannot drag the composition), arrow pointer, "Any agent" MCP beat, side captions that dim the chat, longer holds on the messages to read. Sound: scripts/demo_audio.py synthesizes taps and the connect, confirm, approve and deny cues; the bed is Kevin MacLeod's "Wallpaper" (CC BY 4.0, docs/demo/music/README.md, credit on the outro card and in the README). Take 13: 50 s, 38 cues, mean -22 dB. Commits 35aaf1f, b17650a, 61157da, bd54ed9.

2026-09-25 david_orch: revised the README with an overview, purchase flow, architecture explanation and classifier evidence. Replaced the five-page carousel with two object-motion GIFs. `python3 .cotal/build_classifier_motion.py` exited 0; Pillow readback measured 6480 ms and 6640 ms. Chrome loaded all eight README images, including contributor avatars; at a 390 px viewport the document width was 390 px. Local links resolved, quick-start commands remained unchanged and `git diff --check` passed. The animations are labelled illustrations; displayed Jev probabilities match the recorded diagnostic response. No application code, live purchase, provider call, test suite or linter.

2026-09-25 david_orch: followed up on the README review with the recorded user-flow GIF under How it works, a technical component diagram with runtime entry points, protocols, module boundaries and storage, and model input/output animations. `python3 .cotal/build_technical_architecture.py` and `python3 .cotal/build_model_flow.py` exited 0; both classifier loops are 6640 ms. Values are from classifier-composition-2026-09-25.json: AU0001 CatBoost 0.029953195661059726, Jev spend ordinary/unclear/unusual 0.75/0.16/0.09 and activity 0.74/0.02/0.24. The recorded approval is distinguished from the illustrated rule-failure branch. CatBoost escalation was disabled in that recording. Static alternatives, valid local links, unchanged quick-start commands and the newer demo music credit were retained. No live model call, customer answer, application change, test suite or linter.
2026-09-25 12:14 david_main: demo video take 20 (37 s) published as release asset https://github.com/davidfarah2003/Bucheggang/releases/download/demo-2026-09-25/leash-user-flow.mp4 (tag demo-2026-09-25); the README links it and shows the gif inline. The mp4 also stays in the tree at docs/demo/. Take 20 opens on the rules ("Alex sets the rules first"), the MCP beat says the agent does the rest, the closing explainer says the models are bound to the confirmed policy and can add a question only. Focus ring on overlays is keyboard-only now so a red outline no longer suggests a decline. Commit eb55e1a.
2026-09-25 12:20 david_main: take 21, 36 s, sound cues cut to pair, confirm, approve, decline and outro; tool completions, zooms and the opening are silent, taps at half gain. Release asset replaced. @9bc3af8

2026-09-25 david_orch: first quality export rebuilt the How it works GIF directly from the then-current 1600x900 take 20 release MP4, using the updated scripts/record_demo.py export settings: 1280x720, 15 fps, 256-colour palette and ordered dithering. ffprobe readback: 548 frames, 36.54 s, 52,207,345 bytes; the prior GIF was 800x450, 10 fps and 128 colours. Inspected matching Wallet frames for text clarity and the rendered README button. Added a centred Watch the demo SVG link to the existing full-quality release asset; its MP4 SHA-256 still matches the release digest. An error-diffusion encode was 162 MB and was discarded. Local links and git diff --check pass. No new recording or customer action.

2026-09-25 david_orch: stopped the owned stale Wallet server b6utlo1x4 after observing an empty prepared store, healthy but idle service and source e8d1798; its port is closed. Replayed the current main runtime across all 45 public inputs, 11 approve/32 decline/2 step_up, with exact decision/reason parity. A fresh read-only runner GET /v1/bootstrap returned HTTP 200 in 0.185 s with 10 scenarios and zero OpenRouter accessor calls. The first attempt stopped before HTTP because this worktree lacked .env; linked the existing ignored repository configuration and made one successful request. Private response remains in .cotal with mode 0600. No simulator run, mandate confirmation or purchase answer; whole-application verification remains incomplete.

2026-09-25 david_orch: rebased the quality export and README button over main c328466, preserving take 21 and its audio changes. Regenerated the GIF from that take using the same 1280x720/15 fps/256-colour settings: 539 frames, 35.94 s, 51,482,208 bytes. Full ffmpeg decode exited 0. The source MP4 digest matches the current release asset; no source-video or audio edit was made. Both writers' plan Log entries and the release link were retained.
2026-09-25 12:32 david_main: take 22, 37 s, recorded at 2x through the Chrome screencast (Page.startScreencast, png frames, concat demuxer), encoded 1920x1080 crf 16; audio aac_at 256k 48 kHz with a limiter, cue notes no longer cut mid-ring. GIF 1280x720 15 fps 256 colours from the same take. Release asset replaced. @24cf8d2

2026-09-25 david_orch: replaced the dark Watch the demo button and offset shadow with a pale outlined pill, centred label and smaller play icon. Inspected the rendered README in Chrome. Coordinated media ownership with david_main and left the new capture pipeline untouched. ffprobe confirms the source video is 1920x1080 H.264 at 30 fps; its SHA-256 matches the current release asset. No new media export or recording from this session.
2026-09-25 12:43 david_main: take 25, 38 s. Declined buy shows a red cross and red bubble in the agent panel, the step-up an amber question; deny cue is two low falling notes. Explainer steps rewritten in plain words and step 1 says the rules are checked by code, no model. Pipeline scroll uses scrollIntoView (smooth, no wheel overshoot), jitter at the bottom gone. Release asset replaced. @3b3ba95
2026-09-25 12:49 david_main: take 26, 38 s. Every caption, chat bubble and explainer step rewritten as a plain sentence (no clipped fragments, no colon labels). Release asset replaced. @7288635
2026-09-25 13:10 david_main: labels decided. The strict candidate is docs/eval/labels.csv, the ask candidate deleted. Reason: both replay policies say ask when uncertain, but the six rows are not uncertain. AU0016 fails a stated rule (purchase count), and the SCEN0003 five are a shop this card has never used, which the confirmed rule (shops I have used before) excludes outright. A rule failure is a decline in the contract, and asking the customer to override a rule they wrote is what the design forbids. Report: uv run python scripts/report.py --replay docs/eval/replay-2026-09-25-all.csv --labels docs/eval/labels.csv --labels-mode sequential --live <four docs/eval/live-*.csv> --output docs/eval/report-labelled.md. Result 43/45 label agreement, autonomous error 1/43, disagreements AU0035 and AU0040 as recorded at 03:25.
