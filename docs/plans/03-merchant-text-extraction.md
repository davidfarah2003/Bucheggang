...
Observed with `extract_item` and `extract_event`. The merged conflict contract now covers the marked contradiction cases below. Other behavior changes that need new fact meanings require an explicit contract decision.
...
- Contradictory return windows and sizes now emit the contract's `merchant_text_contradiction` marker with an unknown value and no source. Other unparsed or negated claims can still be misread; the regex patterns do not establish general semantic understanding.
- Negation is not handled. "Not a subscription, one-off payment" gives `is_subscription: true` from the word "subscription".
- Catalogue/event name or category disagreement now makes `product_type` unknown with `catalogue_event_mismatch`. A category disagreement also marks `is_gift_card` or `is_subscription` unknown when either category names that type. The live runner (`src/leash/runner/loop.py`, `extract_with_budget`) passes no catalogue, so this check applies to paths that provide one; live `product_type` still comes from merchant text.
...
2026-09-25 single-agent takeover: `extract_item` now emits closed conflict markers for different stated return windows or sizes and for catalogue/event name or category mismatch, leaving affected fields unknown without sources. Validated three focused `PurchaseFacts` records and a category mismatch, and evaluated AU0012 with an added contradictory "No returns" statement: `step_up`, `return_terms_missing|fact_conflict`. `scripts/measure_extract.py` still reports 145/145 annotated fields and 7/7 authored instruction lines. Full 45-attempt sequential replay completed 11 approve, 32 decline, 2 step_up with zero decision or reason-code differences from `docs/eval/replay-2026-09-25-all.csv`. No live simulator or customer answer was used for this change.

2026-09-25 david_orch: checked main3bf1791 and found `size EU 43; EU 44` incorrectly returned size43/matches_request true because the generic size match skipped EU parsing. `_size` now includes all recognized EU/UK claims when checking conflicts and excludes bare unit labels from generic sizes. Ten direct extract_item calls validated against PurchaseFacts: contradictory mixed wording returned unknown size/match with merchant_text_contradiction and no size source; repeated size EU43/EU43 stayed43, standalone size43/EU43/UK9 kept their representations, and size UK9 returned UK9. No cross-unit conversion is inferred. The actual AU0012 replay Event approved unchanged; appending EU44 produced step_up with item_mismatch|fact_conflict. Existing76-line corpus measured145/145 annotated fields and7/7 authored instruction cases; full45 replay remained11 approve/32 decline/2 step_up with exact decision/reason equality. The first probe incorrectly wrapped PurchaseFacts in an items envelope and failed schema validation; corrected the probe to validate each returned record, without an application change. No provider/simulator calls, customer answers, test suite or linter. Private measurements: .cotal/mixed-size-observations.json and .cotal/mixed-size-engine.json.
