...
| M1 personal-history features and deterministic integration | M1a history and model-off replay ready for review; full M1 still waits on engine-owned P1/P2 contract integration | [M1 observation](../eval/classifier/m1-report.md), [hist-1 schema](../eval/classifier/feature-schema.md) | M1a review pending |
| M2 offline behavioural-model comparison | split frozen and optional training dependencies installed; fitting waits for M1a history review | [split manifest](../eval/classifier/split-manifest.json), `pyproject.toml` classifier group | not frozen |
| M3 Jev semantic assessments | independent history-only provider adapter implemented and called once; assessment composition and model check pending | `src/leash/engine/classifier/jev.py` | not frozen |
