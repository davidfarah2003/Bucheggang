...
The deadline implementation (#50) and shared assessment contracts (#47) have since merged. #58 is being rebased onto those prerequisites; no model-enabled startup is added by this branch. The challenge credential rotation and genuine customer-flow checks remain unresolved.

On code5ad01c2 based on main0e73499, after #47 and #50 were merged, replay again returned45 rows11/32/2 (local total p50 0.232ms, p99/max0.517ms). Genuine-receipt recovery after process exit17 again produced one CHF18 approval/history and completed step-up/journal. The merged #65 Unicode fix also passed real HTTP: login200, global GET200/version0, PUT200/version1 and read-back version1 for the previously failing80-character name. Encoded glob IDs still returned422. No simulator/provider request, invented ownership or new customer answer was involved.
