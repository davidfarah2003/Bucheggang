...
Pairing. `begin_pairing` creates a code of 16 random bytes (128 bits), URL-safe, valid for 5 minutes, and stores only its digest with the agent label and the fixed scope set. The customer opens the link while logged in; the Wallet shows the label, the scopes and the expiry and offers Approve. Approval writes `account_id` and `status: approved` under the file lock. `complete_pairing` reads the record under the lock, requires `status: approved` and an unexpired record, sets `status: consumed`, and creates the agent record with a token of 32 random bytes (256 bits). The token is returned once, in the tool result over the transport, and is never stored in clear. A code that is unknown, expired or consumed raises `PairingUnknown`; the API side returns 404 for the same states. Expired pairing files are deleted on every `begin_pairing`. The pairing code in the link is not a credential: without the customer's session it approves nothing, and after five minutes or one use it is dead.
...
| Tool | Input → returns |
| --- | --- |
| `begin_pairing` | `{ agent_label: str }` (1 to 80 characters, shown to the customer) → `{ pairing_code, expires_at, scopes }`. The agent hands the customer `/app/?pair=<pairing_code>`. No token needed |
| `complete_pairing` | `{ pairing_code: str }` → `{ agent_token, agent_id, account_id, scopes }` exactly once, after the customer approved in the Wallet; before approval raises `PairingPending`, after consumption or expiry raises `PairingUnknown`. No token needed |
