...
Open `http://127.0.0.1:8000/app/`. The Wallet uses a local demo username. After an MCP-compatible shopping agent creates a draft in the same `LEASH_POLICY_STORE`, open `http://127.0.0.1:8000/app/?draft_id=<draft_id>` to review it. Configure the agent's policy MCP process with that same `LEASH_POLICY_STORE` value; the command and a client config block are in `docs/mcp-client.md`.
